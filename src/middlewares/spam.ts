 ...
 *
 * Callers must treat this as "accept and discard", not "reject". A bot that gets
 * an error learns which field gave it away and retries without it; one that gets
 * a success moves on. The cost of being wrong is a dropped message, which is why
 * neither check fires on anything a person plausibly does.
 */
export function looksAutomated(body: Record<string, unknown>): boolean {
	...
	if (elapsed < MIN_FILL_MS && elapsed > -MIN_FILL_MS) return true;

	return elapsed > MAX_FILL_MS;
}
